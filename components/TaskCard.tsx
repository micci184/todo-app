        ...
        >
          ...
          <label className="grid gap-1.5">
            ...
            <textarea
              ...
              rows={3}
              className="resize-none rounded-md border border-slate-300 bg-white px-3 py-2 text-base text-slate-950 shadow-sm outline-none transition focus:border-slate-500 focus:ring-2 focus:ring-slate-200 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-50 dark:focus:border-slate-400 dark:focus:ring-slate-800 sm:text-sm"
            />
          </label>
          <div className="flex flex-wrap justify-end gap-2">
            <button
              ...
              className="inline-flex h-10 flex-1 items-center justify-center rounded-md border border-slate-300 bg-white px-3 text-sm font-medium text-slate-700 transition hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800 sm:h-9 sm:flex-none"
            >
              キャンセル
            </button>
            <button
              ...
              className="inline-flex h-10 flex-1 items-center justify-center rounded-md bg-slate-950 px-3 text-sm font-semibold text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:bg-slate-300 dark:bg-slate-100 dark:text-slate-950 dark:hover:bg-white dark:disabled:bg-slate-700 dark:disabled:text-slate-400 sm:h-9 sm:flex-none"
            >
              保存
            </button>
          </div>
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <h3 className="min-w-0 break-words text-sm font-semibold leading-6 text-slate-950 dark:text-slate-50">
          {task.title}
        </h3>
      </div>
      {task.description ? (
        ...
        {isConfirmingDelete ? (
          ...
          <div className="flex flex-wrap items-center justify-end gap-2 opacity-100 transition sm:opacity-80 sm:group-hover:opacity-100 sm:group-focus-within:opacity-100">
            <button
              ...
              className="inline-flex h-9 items-center justify-center rounded-md px-3 text-xs font-medium text-slate-500 transition hover:bg-slate-100 hover:text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-400 dark:text-slate-400 dark:hover:bg-slate-900 dark:hover:text-slate-100 sm:h-8 sm:px-2.5"
            >
              編集
            </button>
            <button
